...
/**
 * Tag membership, INJECTED rather than imported.
 *
 * The estimator's signals (mass land denial, extra turns, tutors,
 * counterspells) are tag lookups, and in the frontend those live in
 * `deck-builder/services/tagger/client.ts` — a ~1000-line module that lazily
 * `fetch`es `tagger-tags.json` into a module-global `tagSets`. Importing it
 * here would drag network I/O into an isomorphic package AND, worse, fail
 * SILENTLY on the server: every predicate there returns `false`/`null` when
 * `tagSets` is null, so a backend caller would score every deck as having no
 * mass land denial, no extra turns and no roles. It would not throw — it would
 * just be wrong.
 *
 * So the two primitives are parameters. Each caller binds its own source: the
 * frontend passes the tagger client, the backend passes a lookup over its own
 * copy of the tag data. This mirrors `gameChangerNames`, which was already
 * threaded in as a parameter for exactly this reason.
 */
export interface TagLookup {
  /** Does `name` carry the tagger tag `tag` (e.g. 'tutor', 'counterspell')? */
  hasTag(name: string, tag: string): boolean;
  /** Primary role, or null when untagged / tag data unavailable. */
  getCardRole(name: string): RoleKey | null;
  /** otag:mass-land-denial membership. */
  isMassLandDenial(name: string): boolean;
  /** otag:extra-turn membership. */
...
/**
 * A Spellbook combo as the deck's combo detection reports it. Structurally
 * identical to the frontend's `DetectedCombo` (deck-builder/types) — redeclared
 * here so the package stays dependency-free; the frontend type is assignable.
 */
...
export function isMassLandDenialFloor(name: string, tags: TagLookup): boolean {
  return tags.isMassLandDenial(name) && !MLD_FALSE_POSITIVES.has(name);
}
...
export function estimateBracket(
  ...
  for (const name of allCardNames) {
    ...
    if (isTutor(name, tags)) tutors.push(name);
  }
