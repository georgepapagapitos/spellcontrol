...
- **Scan paper cards** — add cards to the collection by pointing a phone camera at them.
...
- `ADMIN_EMAILS` — optional, comma-separated list of emails that seed the `admin` role. It exists to mint the FIRST admin on a fresh database: a match promotes only once that address is **verified** on the account (an unverified address is just a string someone typed into a form), and it is additive, so removing an address never demotes anyone. Every permission check reads `users.role`; ongoing grants and revokes go through the admin panel's role control, which is keyed on the immutable user id rather than on any label a user can change. Admins see an extra "Admin — manage users" card on the Settings page, where they can list users, see per-account storage size, grant or revoke admin, delete accounts, grant individual accounts the AI features (with an optional per-user daily limit) while `AI_PUBLIC` is unset, and read an estimated AI spend (today / 7 days / 30 days, plus a 30-day per-user column).
...
- `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `OAUTH_WEB_REDIRECT_URI` — optional. Enable "Continue with Google" SSO when all three are set; with any unset, the `/api/auth/google*` routes return 503 and the frontend hides the button. Create an OAuth 2.0 Client (Web application type) in the Google Cloud Console and register the redirect URI you use. See `.env.example` for the values you'll want in dev vs prod.
- `APP_HTTPS_DEEPLINK_BASE` — optional. Origin baked into the links the backend mails out (email verification, password reset); defaults to `https://spellcontrol.com`. Set it in local development so those links point at your own stack.
...
| **A cross-origin build is long-poll only.** `EventSource` cannot send credentials cross-origin without CORS, and the backend sends none — the session cookie is `SameSite=Lax` (deliberate CSRF protection in `backend/src/auth.ts`), so credentialed CORS was rejected. | Long-poll rides the same `fetch` path everything else uses. Each update costs one held request (25s hold, 250ms cycle floor), which is fine for a life pad and a low-frequency board. | A backgrounded tab drops the held request without an error event; every return to visible forces a transport restart plus a catch-up refresh. A failed round-trip retries after 5s while the 2.5s poll carries the game. | Revisit if a same-origin deploy ever needs it. |
...
| **The horde step log is client-asserted.** `horde-step`/`horde-undo` are validated shape-and-range only (a real seat, a whitelisted step kind, damage 0-999); nothing checks a `take`/`damage` against the horde deck itself.                                                    | Same trust model as hidden zones above — no server-side horde deck, no rules engine — and every device replays the identical log against its own copy of the deck.                                                                                                              | Any survivor's device can record damage or a Take the horde never dealt; accepted at a table of friends, like the rest of the table.                                                                                                                                                                                                                  | Server-side replay of the horde deck.                                                                            |
