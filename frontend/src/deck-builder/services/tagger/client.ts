...
// Positive-evidence patterns per role (E77 iter-4 sanity layer, used by
// `validateCardRole` below) now live in `@spellcontrol/deck-metrics` as
// `checkRoleEvidence` — the backend's `check_bracket` tool needs the exact
// same gate, and a second hand-maintained copy would drift.
