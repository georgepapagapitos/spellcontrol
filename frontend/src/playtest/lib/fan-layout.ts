...
/**
 * The band down each side of the table the fan must not reach into: the log
...
/** Ceiling however wide the table gets: a fan past this stops reading as a
 ...
 * a 1920px table).
 */
export function fanOverlap(count: number, cardW: number, viewportW: number): number {
  if (count < 2 || cardW <= 0) return MIN_FAN_OVERLAP;
  const half = Math.min(viewportW / 2 - DOCK_RESERVE, viewportW - PILES_RESERVE - viewportW / 2);
  const maxFanWidth = Math.min(viewportW * MAX_FAN_VW, Math.max(cardW, half * 2));
