...
describe('useDecksStore — toggleCardTag', () => {
  let id: string;
  let slotId: string;
  beforeEach(() => {
    id = store().createDeck({ source: 'manual', commander: null });
    slotId = store().addCard(id, sfCard('Sol Ring'));
  });

  it('adds a tag to a mainboard slot and bumps updatedAt', () => {
    const before = store().decks[0].updatedAt;
    store().toggleCardTag(id, slotId, 'Ramp');
    const d = store().decks[0];
    expect(d.cards[0].tags).toEqual(['Ramp']);
    expect(d.updatedAt).toBeGreaterThanOrEqual(before);
  });

  it('accumulates distinct tags without duplicating', () => {
    store().toggleCardTag(id, slotId, 'Ramp');
    store().toggleCardTag(id, slotId, 'Synergy');
    expect(store().decks[0].cards[0].tags).toEqual(['Ramp', 'Synergy']);
  });

  it('toggles a present tag off, keeping the others', () => {
    store().toggleCardTag(id, slotId, 'Ramp');
    store().toggleCardTag(id, slotId, 'Draw');
    store().toggleCardTag(id, slotId, 'Ramp');
    expect(store().decks[0].cards[0].tags).toEqual(['Draw']);
  });

  it('deletes the tags key (not []) when the last tag is removed', () => {
    store().toggleCardTag(id, slotId, 'Ramp');
    store().toggleCardTag(id, slotId, 'Ramp');
    const slot = store().decks[0].cards[0];
  ...
  it('caps at 4 tags — a 5th add is ignored and does not bump updatedAt', () => {
    for (const t of ['Ramp', 'Draw', 'Interaction', 'Removal']) {
      store().toggleCardTag(id, slotId, t);
    }
    const before = store().decks[0].updatedAt;
    const cardsBefore = store().decks[0].cards;
    store().toggleCardTag(id, slotId, 'Wincon');
    const d = store().decks[0];
    expect(d.cards[0].tags).toEqual(['Ramp', 'Draw', 'Interaction', 'Removal']);
    expect(d.updatedAt).toBe(before);
    expect(d.cards).toBe(cardsBefore); // untouched deck object → no sync write
  });

  it('still allows removing (then re-adding) at the cap', () => {
    for (const t of ['Ramp', 'Draw', 'Interaction', 'Removal']) {
      store().toggleCardTag(id, slotId, t);
    }
    store().toggleCardTag(id, slotId, 'Draw'); // off
    store().toggleCardTag(id, slotId, 'Wincon'); // frees a slot for this
    expect(store().decks[0].cards[0].tags).toEqual(['Ramp', 'Interaction', 'Removal', 'Wincon']);
  });

  it('works for sideboard slots', () => {
    const sbSlot = store().addSideboardCard(id, sfCard('Negate'));
    store().toggleCardTag(id, sbSlot, 'Interaction');
    expect(store().decks[0].sideboard[0].tags).toEqual(['Interaction']);
    store().toggleCardTag(id, sbSlot, 'Interaction');
    expect('tags' in store().decks[0].sideboard[0]).toBe(false);
  });

  it('only touches the matching slot', () => {
    const other = store().addCard(id, sfCard('Swamp'));
    store().toggleCardTag(id, slotId, 'Ramp');
    const d = store().decks[0];
    expect(d.cards.find((c) => c.slotId === other)?.tags).toBeUndefined();
  });

  it('is a no-op for an unknown slot or deck (no updatedAt bump)', () => {
    const before = store().decks[0].updatedAt;
    store().toggleCardTag(id, 'nope', 'Ramp');
    store().toggleCardTag('missing-deck', slotId, 'Ramp');
    expect(store().decks[0].updatedAt).toBe(before);
    expect(store().decks[0].cards[0].tags).toBeUndefined();
  });
});
