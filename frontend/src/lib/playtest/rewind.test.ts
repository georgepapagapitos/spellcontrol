...
const SAMPLE_ACTIONS: Record<PlaytestAction['type'], PlaytestAction> = {
  ...
  REVEAL_HAND: { type: 'REVEAL_HAND' },
  SET_HAND_REVEALED: { type: 'SET_HAND_REVEALED', revealed: true },
  DISCARD_RANDOM: { type: 'DISCARD_RANDOM' },
  ...
  ADJUST_ALL_COUNTERS: { type: 'ADJUST_ALL_COUNTERS', cardId: 'x', op: 'inc' },
  ADJUST_PT: { type: 'ADJUST_PT', cardId: 'x', power: 1, toughness: 1 },
  TOGGLE_REVEAL: { type: 'TOGGLE_REVEAL', cardId: 'x' },
  PUT_ON_STACK: { type: 'PUT_ON_STACK', cardId: 'x' },
  RESOLVE_STACK: { type: 'RESOLVE_STACK' },
