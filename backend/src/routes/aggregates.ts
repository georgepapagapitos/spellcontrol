...
aggregatesRouter.get('/trending', aggregatesLimiter, async (_req: Request, res: Response) => {
  ...
  const trendingDecks = await loadTrendingDecks(Date.now());
  ...
  res.json({
    ...
    // No score: it's an internal ranking key, not something to show.
    trendingDecks: trendingDecks.map(({ slug, deckName, commanderName, players }) => ({
      slug,
      deckName,
      commanderName,
      players,
